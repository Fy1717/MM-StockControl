class Product:
    ...
    def update(self, count, reference):
        try:
            query = f"UPDATE {self.table_name} SET count=? WHERE reference=?"
            
            self.db.execute(query, (count, reference))
    ...
    def delete(self, reference):
        try:
            ...
            query = f"DELETE FROM {self.table_name} WHERE reference=?"
            self.db.execute(query, (reference,))
